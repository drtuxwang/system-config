...
for /f %%a in ('dir /b /s ..\pool\software\7zip_*_win*x86.exe') do (
...
if "%PROCESSOR_ARCHITEW6432%"=="AMD64" goto win64
    for /f %%a in ('dir /b /s ..\pool\software\busybox_*_win??x86.7z') do (
        echo Installing "%%a" archive...
        call c:\software\bin\7z.bat x -oc:\software -y "%%a"
    )
    goto install
:win64
    for /f %%a in ('dir /b /s ..\pool\software\busybox_*_win64*x86.7z') do (
        echo Installing "%%a" archive...
        call c:\software\bin\7z.bat x -oc:\software -y "%%a"
    )
:install
