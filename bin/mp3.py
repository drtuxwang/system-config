#!/usr/bin/env python3
"""
Encode MP3 audio using avconv (libmp3lame).
"""
...
class Options(syslib.Dump):
    ...
    def _parseArgs(self, args):
        parser = argparse.ArgumentParser(
                description="Encode MP3 audio using avconv (libmp3lame).")
        ...
        parser.add_argument("files", nargs="+", metavar="file",
                            help='Multimedia file. A target ".mp3" file can '
...
class Encoder(syslib.Dump):
        ...
        if self._options.getFileNew():
            ...
            if len(self._options.getFiles()) > 1:
                ...
                for file in self._options.getFiles():
                    ...
                    number += 1
                self._avconv.setArgs(args + [ "-filter_complex",
                                     maps + "concat=n=" + str(number) + ":v=0:a=1 [out]",
                                     "-map", "[out]" ] + self._avconv.getArgs()[2:])
            self._avconv.extendArgs([ "-f", "mp3", "-y", self._options.getFileNew() ])
            self._run()
            ...
            for file in self._options.getFiles():
                if not file.endswith(".mp3"):
                    print()
                    self._config(file)
                    fileNew = file.rsplit(".", 1)[0] + ".mp3"
                    self._avconv.extendArgs([ "-f", "mp3", "-y", fileNew ])
                    self._run()
                    Media(fileNew).print()
    ...
    def _config(self, file):
        ...
        self._avconv.extendArgs([ "-vn", "-threads",
                                  self._options.getThreads() ] + self._options.getFlags())
        return media
