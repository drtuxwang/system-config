...
app_settings_deb12() {
    NAME="grub-pc"
    VERSION="2.06"
    PORT="linux-x86-glibc_2.36"

    APP_DIRECTORY="${NAME}_$VERSION-$PORT"
    REPO="https://deb.debian.org/debian/pool"
    APP_FILES="
        $REPO/main/g/grub2/grub-pc-bin_2.06-13+deb12u1_amd64.deb
        $REPO/main/g/grub2/grub2-common_2.06-13+deb12u1_amd64.deb
    ...
    APP_SHELL="
        ...
        mv usr/lib/x86_64-linux-gnu/libefivar.so.1.* grub-pc/libefivar.so.1
        cp ${0%.*}/grub.cfg grub-pc/
        cp ${0%.*}/README-grub-pc.md-deb12 README-grub-pc.md
        cp ${0%.*}/install-grub-pc.bash .
        touch -r grub-pc/grub-install grub-pc/grub.cfg
        export XZ_OPT='-9 -e --x86 --lzma2=dict=128MiB --threads=1'
        tar cfJ - grub-pc --owner=0:0 --group=0:0 >> install-grub-pc.bash
...
source "${0%/*}/setup-software.bash" app_settings app_settings_deb12
